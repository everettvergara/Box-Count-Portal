...
use App\Http\Requests\tb_bc_mf_company_request;
use App\Models\tb_bc_mf_company;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class tb_bc_mf_company_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $title;
    ...
    public function index(Request $r)
    {
        ...
        $name = $r['name'];
        $data = tb_bc_mf_company::select('tb_bc_mf_company.*')
            ->when(isset($name), function ($q) use ($name) {
                return $q->where('tb_bc_mf_company.name', 'like', '%' . $name . '%');
            })
            ->sortable()
    ...
    public function store(tb_bc_mf_company_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_bc_mf_company();
    ...
    {
        $datum = tb_bc_mf_company::findOrFail($id);
    ...
    public function update(tb_bc_mf_company_request $r, $id)
    {
        $datum = tb_bc_mf_company::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
