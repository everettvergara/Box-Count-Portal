<?php

namespace App\Http\Controllers;

use App\Http\Requests\tb_bc_mf_warehouse_portal_request;
use App\Models\tb_bc_mf_warehouse_portal;
use Illuminate\Http\Request;

class tb_bc_mf_warehouse_portal_controller extends Controller
{
    protected $route;
    protected $route_var;
    protected $header_route;
    protected $header_route_var;
    protected $title;
    protected $view_path;
    protected $pk_var;
    protected $pk_val;

    public function __construct()
    {
        $this->route = 'warehouse-portals';
        $this->route_var = 'warehouse_portal';
        $this->header_route = 'warehouses';
        $this->header_route_var = 'warehouse';
        $this->title = 'Warehouse Portal';
        $this->view_path = 'tb_bc_mf_warehouse_portal';
        $this->pk_var = 'warehouse_id';
        $this->pk_val = 'warehouse';
    }

    public function create(Request $r)
    {
        $head = view($this->view_path . '.create.head')->render();
        $form_fields = view($this->view_path . '.create.form_fields', [
            $this->pk_var       => $r->{$this->pk_val},
        ])->render();
        $scripts = view($this->view_path . '.create.scripts')->render();
        return view('base.detail.create', [
            'route'             => $this->route,
            'header_route'      => $this->header_route,
            'header_route_var'  => $this->header_route_var,
            'header_route_val'  => $r->{$this->pk_val},
            'title'             => $this->title,
            'head'              => $head,
            'form_fields'       => $form_fields,
            'scripts'           => $scripts,
        ]);
    }

    public function store(tb_bc_mf_warehouse_portal_request $r)
    {
        $validatedData = $r->validated();
        $datum = new tb_bc_mf_warehouse_portal();
        $datum->fill($validatedData);
        $datum->save();
        return redirect()->route($this->route . '.show', [$this->route_var => $datum->id])->with('status', 'Success!');
    }

    public function show($id)
    {
        $datum = tb_bc_mf_warehouse_portal::findOrFail($id);
        $form_fields = view($this->view_path . '.show.form_fields', [
            'datum'             => $datum,
        ])->render();
        return view('base.detail.show', [
            'route'             => $this->route,
            'route_var'         => $this->route_var,
            'route_val'         => $datum->id,
            'header_route'      => $this->header_route,
            'header_route_var'  => $this->header_route_var,
            'header_route_val'  => $datum->{$this->pk_var},
            'title'             => $this->title,
            'form_fields'       => $form_fields,
        ]);
    }

    public function edit($id)
    {
        $datum = tb_bc_mf_warehouse_portal::findOrFail($id);
        $form_fields = view($this->view_path . '.edit.form_fields', [
            'datum'             => $datum,
        ])->render();
        return view('base.detail.edit', [
            'route'             => $this->route,
            'route_var'         => $this->route_var,
            'route_val'         => $datum->id,
            'header_route'      => $this->header_route,
            'header_route_var'  => $this->header_route_var,
            'header_route_val'  => $datum->{$this->pk_var},
            'title'             => $this->title,
            'form_fields'       => $form_fields,
        ]);
    }

    public function update(tb_bc_mf_warehouse_portal_request $r, $id)
    {
        $datum = tb_bc_mf_warehouse_portal::findOrFail($id);
        $validatedData = $r->validated();
        $datum->fill($validatedData);
        $datum->update();
        return redirect()->route($this->route . '.show', [$this->route_var => $datum->id])->with('status', 'Success!');
    }

    public function destroy(Request $r, $id)
    {
        $datum = tb_bc_mf_warehouse_portal::findOrFail($id);
        $pk = $datum->{$this->pk_var};
        $datum->delete();
        return redirect()->route($this->header_route . '.show', [$this->header_route_var => $pk])->with('status', 'Success!');
    }
}
